<template>
  <div class="persons">
    ...
    <div v-if="isLogin()" class="persons-header">
      ...
      <!--</div>-->
        <div style="width:100px;height: 100px;border-radius: 50px;background-color: #FFF2DB;margin:0 auto 20px;">
        ...
        <table style="display:flex;flex-direction: row;border: 0;margin:0;padding: 0;">
          <tr style="flex:1;">
            <td style="position: relative;margin:0 auto;text-align: center;width:25%;">
              <router-link :to="{path: '/orderList',query: {selected:'one'}}" class="order-item-child"><img src="../../images/myOrder/waitPay.png" width="32px;" height="32px;"/>
              <span class="my-badge" v-if="waitPay!=0">{{waitPay}}</span>
              </router-link>
            </td>
          </tr>
          <tr style="flex:1;">
            <td style="position:relative;margin:0 auto;text-align: center;width:25%;"><img src="../../images/myOrder/verifying.png" width="32px;" height="32px;"/>
              <span class="my-badge" v-if="verifying!=0">{{verifying}}</span>
            </td>
          </tr>
          <tr style="flex:1;">
            <td style="position:relative;margin:0 auto;text-align: center;width:25%;">
              <img src="../../images/myOrder/waitReceiver.png" width="32px;" height="32px;"/>
              <span class="my-badge" v-if="waitReceiver!=0">{{waitReceiver}}</span>
            </td>
          </tr>
          <tr style="flex:1;">
            <td style="position:relative;margin:0 auto;text-align: center;width:25%;">
              <img src="../../images/myOrder/myOrder.png" width="32px;" height="32px;"/>
              <span class="my-badge" v-if="myOrders!=0">{{myOrders}}</span>
            </td>
          </tr>
        </table>
        <table class="my-orders-container" style="padding: 0;margin-top:-5px;">
          <tr class="my-order-item">
            <td style="position: relative;width:25%;text-align: center;">
              <router-link :to="{path: '/orderList',query: {selected:'one'}}" class="order-item-child"><span class="common-color">待付款</span></router-link>
              <!--<mt-badge class="badge" size="small" type="error">{{waitPay==0?'':waitPay}}</mt-badge>-->
            </td>
          </tr>
          <tr class="my-order-item">
            <td style="position: relative;width:25%;text-align: center;">
              <router-link :to="{path: '/orderList',query: {selected:'two'}}" class="order-item-child"><span class="common-color">确认中</span></router-link>
              <!--<span class="my-badge" v-if="verifying!=0">{{verifying}}</span>-->
            </td>
          </tr>
          <tr class="my-order-item">
            <td style="position: relative;width:25%;text-align: center;">
              <router-link :to="{path: '/orderList',query: {selected:'three'}}" class="order-item-child"><span class="common-color">待发货</span>
              </router-link>
              <!--<span class="my-badge" v-if="waitReceiver!=0">{{waitReceiver}}</span>-->
            </td>
          </tr>
          <tr class="my-order-item">
            <td style="position: relative;width:25%;text-align: center;">
              <router-link :to="{path: '/orderList',query: {selected:'four'}}" class="order-item-child"><span class="common-color">我的订单</span>
              </router-link>
            </td>
          </tr>
        </table>
      </div>
      <mt-button size="large" type="primary" @click.navite="toLogOut" style="position: fixed;left: 0;bottom: 45px;background-color: #EB89AA">
        退出当前账户
      </mt-button>
    </div>
    ...
    <div v-if="isLogin()" class="update-pwd-container-person">
      ...
      <mt-cell title="  实名认证" to="authIdentity" is-link>
        <!--<span>信息</span>-->
        <img slot="icon" src="../../images/myOrder/shop-auth05.png" width="24" height="24">
      </mt-cell>
    </div>
